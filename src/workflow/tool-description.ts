...
- workflow(nameOrRef: string | {scriptPath: string}, args?: any): Promise<any> — run another workflow inline as a sub-step and return whatever it returns. Pass a name to invoke a saved workflow (same registry as {name: "..."}), or {scriptPath} to run a script file you Wrote earlier. The child shares this run's concurrency cap, agent counter, abort signal, and token budget — its agents appear chained under their ancestors ("▸ outer › inner") in /agents → Workflows and its tokens count toward budget.spent(). The args param becomes the child's \`args\` global. Nesting is bounded by maxWorkflowDepth (default 6): a workflow() past that depth throws naming the limit and the workflow. Throws on unknown name / unreadable scriptPath / child syntax error; catch to handle gracefully.
...
The tool result includes a runId. To resume after a pause, kill, or script edit, relaunch with SubagentWorkflow({scriptPath, resumeFromRunId}) — unchanged agent() calls return cached results instantly; an edited/new call, and everything downstream of it, runs live. Same script + same args → 100% cache hit, including pipelines whose items finish in a different order. A call is identified by where it sits in the script, not by when it finished: each parallel() thunk, each pipeline() item's whole stage chain and each nested workflow() body is its own chain, so editing one item's prompt re-runs that item and leaves its siblings cached. A journaled failure is never replayed, so resuming a run that died at agent 5 retries exactly agent 5. A run id only resolves in the session that ran it — stop it from /agents → Workflows first, and the run must have finished. From a new session, pass resumeFromKey instead (the run's 64-character resume key, sha256 of its script plus its args, reported as \`Resume key\` when the run started) with identical script and args: a run id wins when both are given (a malformed key still errors). Before diagnosing why a completed workflow returned an empty or unexpected result, Read the run's \`<run id>.workflow.jsonl\` beside its script — it records each agent's actual return value; do not assume cached results are non-empty. Date.now()/Math.random()/new Date() are unavailable in scripts (they would break this) — stamp results after the workflow returns, or pass timestamps via args.`;
